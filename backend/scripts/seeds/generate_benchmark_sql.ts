 ...
 *
 * Generates a SQL file with all 3,024 benchmark INSERT statements
 * for sport_normative_data. Uses the same data generation logic as
 * seedFootballBenchmarks() but outputs pure SQL instead of hitting Supabase.
 *
 * Usage:  npx tsx scripts/seeds/generate_benchmark_sql.ts
 * Output: scripts/seeds/benchmark_seed.sql
 */

import * as fs from "fs";
import * as path from "path";

// ── Constants (mirrored from football_benchmark_seed.ts) ─────────
...
function applyLevelScale(
  ...
  const p50 =
    direction === "higher_better"
      ? baseline.p50 * factor
      : baseline.p50 / factor;
  return { p50, sd: baseline.sd * 1.15 };
}
...
function generateRows(): BenchmarkRow[] {
        ...
        for (const gender of GENDERS) {
          for (const level of LEVELS) {
            ...
            rows.push({
              ...
              direction: metric.direction === "higher_better" ? "higher" : "lower",
...
function rowToValues(r: BenchmarkRow): string {
  return `(gen_random_uuid(), ${escapeSQL(r.sport_id)}, ${escapeSQL(r.metric_name)}, ${escapeSQL(r.unit)}, ${escapeSQL(r.attribute_key)}, ${escapeSQL(r.direction)}, 13, 23, '[]'::jsonb, '[]'::jsonb, now(), now(), ${escapeSQL(r.position_group)}, ${escapeSQL(r.age_band)}, ${escapeSQL(r.gender)}, ${escapeSQL(r.metric_key)}, ${escapeSQL(r.metric_label)}, ${r.p10}, ${r.p25}, ${r.p50}, ${r.p75}, ${r.p90}, ${r.mean_val}, ${r.std_dev}, ${r.sample_size}, ${escapeSQL(r.source_ref)}, ${escapeSQL(r.competition_lvl)})`;
}
...
function generateSQL(): string {
  ...
  parts.push(`-- Football Benchmark Seed SQL`);
  parts.push(`-- Generated: ${new Date().toISOString()}`);
  parts.push(`-- Total rows: ${rows.length}`);
  parts.push(`-- 9 positions x 7 age bands x 2 genders x 2 levels x 12 metrics = 3,024`);
  parts.push(``);
  parts.push(`BEGIN;`);
  parts.push(``);
  ...
  for (let i = 0; i < rows.length; i += BATCH) {
    ...
    parts.push(`-- Batch ${batchNum}/${totalBatches} (rows ${i + 1}–${i + batch.length})`);
    parts.push(`INSERT INTO sport_normative_data`);
    parts.push(`  (id, sport_id, metric_name, unit, attribute_key, direction, age_min, age_max, means, sds, created_at, updated_at, position_group, age_band, gender, metric_key, metric_label, p10, p25, p50, p75, p90, mean_val, std_dev, sample_size, source_ref, competition_lvl)`);
    parts.push(`VALUES`);
    parts.push(`  ${values}`);
    parts.push(`ON CONFLICT (sport_id, metric_key, position_group, age_band, gender, competition_lvl)`);
    parts.push(`DO UPDATE SET`);
    parts.push(`  p10 = EXCLUDED.p10, p25 = EXCLUDED.p25, p50 = EXCLUDED.p50,`);
    parts.push(`  p75 = EXCLUDED.p75, p90 = EXCLUDED.p90, mean_val = EXCLUDED.mean_val,`);
    parts.push(`  std_dev = EXCLUDED.std_dev, sample_size = EXCLUDED.sample_size,`);
    parts.push(`  metric_label = EXCLUDED.metric_label, source_ref = EXCLUDED.source_ref,`);
  ...
  parts.push(`COMMIT;`);
  parts.push(``);
  parts.push(`-- Verify row count`);
  parts.push(`SELECT count(*) AS benchmark_rows FROM sport_normative_data WHERE sport_id = 'football';`);
...
function main() {
  ...
  const outPath = path.resolve(__dirname, "benchmark_seed.sql");
  fs.writeFileSync(outPath, sql, "utf-8");
  console.log(`Written ${sql.length.toLocaleString()} chars to ${outPath}`);

  // Quick sanity check
  const rows = generateRows();
  console.log(`Total rows generated: ${rows.length}`);
  const insertCount = (sql.match(/INSERT INTO/g) || []).length;
  console.log(`INSERT statements: ${insertCount} (batches of 100)`);
}
