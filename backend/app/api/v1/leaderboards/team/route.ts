...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Team leaderboard — for now returns the same global list
    // since team_id column needs to be added to users table
    ...
    return NextResponse.json(
      { leaderboard: ranked, userRank, type: "team" },
      { headers: { "api-version": "v1" } }
    );
  } catch (err) {
    console.error('[GET /api/v1/leaderboards/team] error:', err);
