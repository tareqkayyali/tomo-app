-- Migration 071: GDPR Article 17 Deletion Pipeline
--
...
--   3. PURGE. pg_cron (migration 072) runs public.tomo_run_deletion_purge()
...
-- Admin check uses the enterprise RBAC model (migration 067 dropped
-- users.is_admin). Only super_admin or institutional_pd roles on an
-- active organization_memberships row can read tombstones.
