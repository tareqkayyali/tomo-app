...
export function getBlockedRanges(
  ...
  for (const evt of events) {
    ...
    blocked.push({
      ...
      endMin: Math.min(end + gap, config.dayEndHour * 60),
    });
  }
...
// ── Config from Effective Rules (rule engine → scheduling engine bridge) ──

/**
 * Build a SchedulingConfig from EffectiveRules + school schedule.
 * Bridges the schedule rule engine (MyRules) with the scheduling engine (generators).
 */
