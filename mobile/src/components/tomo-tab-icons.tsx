 ...
 *
 * Designer handoff (geometry + color stops, often % / objectBoundingBox):
 ...
 *     and mis-resolves RadialGradients + url(#id) refs, so the Tomo sphere
 *     can render fully white. We inject raw markup via dangerouslySetInnerHTML
 *     so the browser paints it natively.
 *   - On NATIVE, SvgXml is used. It does not match browser/SVG spec for
 *     objectBoundingBox + percentage gradient coordinates the same way;
 *     Expo Go can show flat, wrong, or unchanged-looking fills. Gradients
 *     below use gradientUnits="userSpaceOnUse" with numeric coordinates
 *     (same as the pre-handoff embedded assets) while paths/circles match
 *     the handoff files.
 */
...
// the browser parses it natively (preserves gradient url(#id) refs).
