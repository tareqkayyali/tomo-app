...
export function DayGrid({
  ...
  // ── School hours zone (show on school days only) ──
  const schoolZone = useMemo(() => {
    if (!schoolSchedule?.startTime || !schoolSchedule?.endTime || !schoolSchedule?.days?.length) {
      return null;
    }
    // Check if selectedDay is a school day (0=Sun..6=Sat)
    const dayOfWeek = selectedDay.getDay();
    if (!schoolSchedule.days.includes(dayOfWeek)) return null;

    const startMin = timeToMinutes(schoolSchedule.startTime);
    const endMin = timeToMinutes(schoolSchedule.endTime);
    if (startMin >= endMin || startMin < START_HOUR * 60 || endMin > END_HOUR * 60) return null;

    const top = minutesToY(startMin);
    const height = ((endMin - startMin) / 30) * SLOT_HEIGHT;
    return { top, height, startTime: schoolSchedule.startTime, endTime: schoolSchedule.endTime };
  }, [schoolSchedule, selectedDay]);
            ...
            >
              {!isOccupied && !readOnly && !locked && (
                <View style={styles.emptySlotHint}>
                  <SmartIcon name="add" size={14} color={colors.textInactive + '60'} />
                </View>
              )}
            </Pressable>
          );
    ...
    >
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
          ...
          <SmartIcon name="move-outline" size={14} color={colors.textSecondary} />
        </View>
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
          ...
          {!readOnly && !locked && event.startTime && (
            <SmartIcon name="reorder-three-outline" size={14} color={colors.textInactive + '80'} />
          )}
        </View>
        ...
        {isCompleted && !readOnly && !locked && height > 44 && (
          ...
          >
            <SmartIcon name="arrow-undo" size={10} color={typeColor} />
